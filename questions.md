...
- rune (псевдоним int32) - кодовое целое число Unicode для символа
...
Помимо этого, в реализации самой мапы мы начинаем итерироваться по мапе не с первого значения, а с рандомного.
...
Нужно поменять местами поля структуры, которые занимают в памяти меньше одного машинного слова, а также распологать поля в структуре в таком порядке, чтобы соседние с ними занимали не более одного машинного слова. Например:
...
Суммарный вес - 12. Оптимизировать ее можно, поменяв местами второе и третье поле.
...
А у этой структуры уже суммарный вес будет 8.

### 39. Зачем используется ключевое слово defer в Go?
Ключевое слово defer используется для отложенного вызова функции. При этом, место объявления одной инструкции defer в коде никак не влияет на то, когда та выполнится.

Функция с defer всегда выполняется перед выходом из внешней функции, в которой defer объявлялась. При этом функция defer работает с теми данными, которые были до ее вызова. То есть, если мы, например, создадим переменную ***a := 7***, напишем ее отложенный вызов, а потом изменим значение a на 5, то выведется все равно значение семь.

### 40. Каким образом выполняется функция с defer?
Функция с defer всегда выполняется перед выходом из внешней функции, в которой defer объявлялась. При этом функция defer работает с теми данными, которые были до ее вызова. То есть, если мы, например, создадим переменную a := 7, напишем ее отложенный вызов, а потом изменим значение a на 5, то выведется все равно значение семь.

### 41. Каков порядок возврата при использовании несколько функций с defer в рамках одной внешней функции?
...
### 42. Что такое gofmt и goimports?
gofmt - это стиль форматирования программы Go: инструмент, приводящий код к единому формату

goimports - инструмент, управляющий вставкой и удалением объявлений импорта

### 43. Есть ли goto в Go?
Да, есть.

### 44. Что такое shadowing в Go?
Shadowing - явление, когда переменная из локальной области видимости "перекрывает" переменную из глобальной области видимости. Вызвано это, как правило, одинаковым названием переменных.
